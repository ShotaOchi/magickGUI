...
#' @examples
#' \donttest{
#' interactive_emboss(wizard)
#' }
interactive_emboss <- function(image, range_max_radius = 5, range_max_sigma = 5, resolution = 0.1, return_param = FALSE, scale)
{
  ...
  # make initial output
  iniv <- 0
  initial <- image_emboss(image, iniv, iniv)
  ...
  {
    temp_image <- image_emboss(image, temp_val[1], temp_val[2])
  ...
  return(image_emboss(image_original, val_res[1], val_res[2]))
}
