...
#' }
interactive_blur <- function(image, range_max_radius = 5, range_max_sigma = 5, resolution = 0.1, return_param = FALSE, scale)
{
  ...
  # make initial output
  iniv <- 0
  initial <- image_blur(image, iniv, iniv)
  ...
  return(image_blur(image_original, val_res[1], val_res[2]))
}
