...
interactive_quantize <- function(image, colorspace = "rgb", dither = NULL, treedepth = NULL, range_max = 256, resolution = 1, return_param = FALSE, scale)
{
  ...
  text_label <- "Max: "                           # text shown in label
  ...
  win1.frame1.slider <- tkscale(win1.frame1, from = range_max[1], to = range_max[2], variable = slider_value, orient = "horizontal", length = length_slider, command = command_slider, resolution = resolution, showvalue = 0)
  temp_val <- iniv
  update_image <- function()
  {
    temp_image <- image_quantize(image, temp_val, colorspace = colorspace, dither = dither, treedepth = treedepth)
  ...
  return(image_quantize(image_original, val_res, colorspace = colorspace, dither = dither, treedepth = treedepth))
}
